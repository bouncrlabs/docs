...
Set `BOUNCR_NPM_TOKEN` to the package download token supplied by Bouncr. Add this to your project’s `.npmrc`:

```ini .npmrc
@bouncrlabs:registry=https://registry.npmjs.org/
//registry.npmjs.org/:_authToken=${BOUNCR_NPM_TOKEN}
```
...
The SDK loads its versioned runtime from `cdn.bouncr.io`. The npm token is for installation only; do not pass it to `createCapture`.
